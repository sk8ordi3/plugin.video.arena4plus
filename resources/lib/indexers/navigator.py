...
class navigator:
    ...
    def getMpdLic(self, url, arena4_online_session_value, both_title, image_url, main_title):
        ...
        try:
            ...
            stream_url = re.findall(r'src\":\"(https://.*mpd.*)\"', str(soup_xx))[0].strip()
            
            response_3 = requests.get(stream_url)
            ...
            xbmc.log(f'Arena4Plus | v{version} | Kodi: {kodi_version[:5]}| OS: {os_info} | getMpdLic | lic_url | {lic_url}', xbmc.LOGINFO)
            
            xbmc.log(f'Arena4Plus | v{version} | Kodi: {kodi_version[:5]}| OS: {os_info} | getMpdLic | inputstream.adaptive.license_key | {lic}', xbmc.LOGINFO)
        ...
        try:
            ...
            for item in items:
                ...
                for block in video_blocks:
                    ...
                    img_url = block.find('a')['style'].split("'")[5]
                    category_title = block.find('div', class_='bg-green').text.strip()
                    title_1 = block.find('h3').text.strip()
                    title_2 = block.find('div', class_='xl:text-lg').text.strip()

                    self.addDirectoryItem(item, f'get_mpd_lic&url={fix_videok_link}', '', 'DefaultFolder.png')
    ...
    def doSearch(self, url):
        ...
        if search_text != '':
            ...
            for block in video_blocks:
                ...
                self.getMpdLic(url, None, None, None, None)
